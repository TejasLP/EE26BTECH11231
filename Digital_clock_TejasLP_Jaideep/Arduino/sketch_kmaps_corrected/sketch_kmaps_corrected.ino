  ...
  if (timerRunning) {
    if (millis() - lastTimerTick >= 1000) {
      ...
      if (timerSec == 0 && timerMin == 0 && timerHr == 0) {
        ...
        int secU = timerSec % 10; int secT = timerSec / 10;
        int minU = timerMin % 10; int minT = timerMin / 10;
        int hrU  = timerHr  % 10; int hrT  = timerHr  / 10;

        secU = prev_mod10(secU);
        if (secU == 9) {
          secT = prev_mod6(secT);
          if (secT == 5) {
            minU = prev_mod10(minU);
            if (minU == 9) {
              minT = prev_mod6(minT);
              if (minT == 5) {
                if (hrU == 0) { 
        ...
        timerSec = secT * 10 + secU;
        timerMin = minT * 10 + minU;
        timerHr  = hrT  * 10 + hrU;
      }
    }
  }
...
void adjustDigit(int dir) {
  ...
  if (dir > 0) { 
    switch (selectedDigit) {
      ...
      case 5: hrT = next_mod3(hrT); break;
    }
  } else { 
    switch (selectedDigit) {
      ...
      case 5: hrT = prev_mod3(hrT); break;
    }
  }
